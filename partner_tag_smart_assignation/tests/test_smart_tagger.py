...
class TestSmartTagger(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        # Création de partenaires de test
        ...
        # Référence individuelle pour les tests spécifiques
        cls.michel_fletcher = cls.partners.filtered(
            lambda p: p.name == "Michel Fletcher"
        )
        cls.chao_wang = cls.partners.filtered(lambda p: p.name == "Chao Wang")
        cls.david_simpson = cls.partners.filtered(lambda p: p.name == "David Simpson")
        cls.john_m_brown = cls.partners.filtered(lambda p: p.name == "John M. Brown")
        cls.charlie_bernard = cls.partners.filtered(
            lambda p: p.name == "Charlie Bernard"
        )
